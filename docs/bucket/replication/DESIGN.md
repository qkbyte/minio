...
This document explains the design approach of server side bucket replication. If you're looking to get started with replication, we suggest you go through the [Bucket replication guide](https://github.com/qkbyte/minio/blob/master/docs/bucket/replication/README.md) first.
...
`xl.meta` that is in use for [versioning](https://github.com/qkbyte/minio/blob/master/docs/bucket/versioning/DESIGN.md) has additional metadata for replication of objects,delete markers and versioned deletes.
